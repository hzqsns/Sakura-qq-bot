...
class BiliVideoPlugin(Star):
    ...
    def __init__(self, context: Context, config: dict):
        ...
        # BV号去重：{chat_id: {bvid: timestamp}}，防止同一视频短时间内重复解析
        self._recent_bvids: dict[str, dict[str, float]] = {}
        self._bvid_dedup_ttl = 300  # 5分钟内同一chat不重复处理同一BV号
    ...
    def _is_bvid_duplicate(self, chat_id: str, bvid: str) -> bool:
        """检查BV号是否在去重窗口内已处理过，同时清理过期记录"""
        now = time.time()
        chat_cache = self._recent_bvids.get(chat_id, {})
        # 清理过期
        chat_cache = {k: v for k, v in chat_cache.items() if now - v < self._bvid_dedup_ttl}
        self._recent_bvids[chat_id] = chat_cache

        if bvid in chat_cache:
            return True
        chat_cache[bvid] = now
        return False
    ...
    async def on_all_message(self, event: AstrMessageEvent):
        ...
        # 跳过 bot 自身发出的消息，防止解析自己的回复导致重复触发
        try:
        ...
        # BV号去重：同一chat内短时间内不重复处理同一视频
        chat_id = event.unified_msg_origin or ""
        if self._is_bvid_duplicate(chat_id, bvid):
            self._log(f"[AutoDetect] BV号 {bvid} 在 {chat_id} 中已处理过，跳过")
            return
